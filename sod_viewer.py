...
# ---------- Minimal SOD reader (with animation channel capture) ----------
...
class Mesh:
    ...
    bumpmap: str | None = None
    use_heightmap: bool = True
    assimilation_texture: str = ""
    @classmethod
    def from_file(cls, file, sod_version):
        self = cls()
        if sod_version >= 1.7:
            self.material = Identifier.from_file(file).name
        else:
    ...
    def from_file_path(cls, file_path):
        ...
        with open(file_path, "rb") as file:
            ...
            for _ in range(num_mats):
                name = Identifier.from_file(file).name
                file.read(12*3 + 4 + 1)  # placeholder blob (matches our writer)
                if self.version >= 1.9: file.read(1)
                materials[name] = {}
            ...
            # animations
            num_anim = struct.unpack("<H", file.read(2))[0]
            for _ in range(num_anim):
                name = Identifier.from_file(file).name
                kf   = struct.unpack("<H", file.read(2))[0]
                blen = struct.unpack("<I", file.read(4))[0]
                atype= struct.unpack("<H", file.read(2))[0]
                if atype == 5:
                    scalars = [struct.unpack("<f", file.read(4))[0] for __ in range(kf)]
                    channels[name] = {'atype': atype, 'scalars': scalars}
                else:
                    mats = [struct.unpack("<12f", file.read(48)) for __ in range(kf)]
                    channels[name] = {'atype': atype, 'mats': mats}
            if self.version not in (1.4, 1.5):
                num_refs = struct.unpack("<H", file.read(2))[0]
                for _ in range(num_refs):
                    _t = struct.unpack("<b", file.read(1))[0]
                    _n = Identifier.from_file(file).name
                    _a = Identifier.from_file(file).name
                    if self.version >= 1.8: file.read(4)
